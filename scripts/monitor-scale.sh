...
docker build -t local.docker.cn:5000/monitor-scale:$TAG -f applications/monitor-scale/Dockerfile applications/monitor-scale
...
#Push the images
docker push local.docker.cn:5000/monitor-scale:$TAG

#Stop the registry proxy
docker stop socat-registry

# Create the deployment and service for the monitor-scale node server
sed 's#local.docker.cn:5000/monitor-scale:latest#local.docker.cn:5000/monitor-scale:'$TAG'#' applications/monitor-scale/k8s/deployment.yaml | kubectl apply -f -
